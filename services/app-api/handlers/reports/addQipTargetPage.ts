...
import { badRequest, notFound, ok } from "../../libs/response-lib";
...
export const addQipTargetPage = async (report: Report, targetInfo: unknown) => {
  ...
  if (targetInfo.qmsReportId) {
    ...
    const qmsReport = await getReport(
      ReportType.QMS,
      report.state,
