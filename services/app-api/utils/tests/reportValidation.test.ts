...
describe("Test invalid reports", () => {
  it("throws an error when validating a report with missing state", async () => {
    await expect(async () => {
      await validateReportPayload(missingStateReport);
    }).rejects.toThrow();
  });
  it("throws an error when validating a report with incorrect status", async () => {
    await expect(async () => {
      await validateReportPayload(incorrectStatusReport);
    }).rejects.toThrow();
  });
  it("throws an error when validating a report with incorrect report type", async () => {
    await expect(async () => {
      await validateReportPayload(incorrectTypeReport);
    }).rejects.toThrow();
  });
  it("throws an error when validating invalid form page object", async () => {
    await expect(async () => {
      await validateReportPayload(invalidFormPageReport);
    }).rejects.toThrow();
  });
  it("throws an error when validating invalid measure page object", async () => {
    await expect(async () => {
      await validateReportPayload(invalidMeasurePageReport);
    }).rejects.toThrow();
  });
  it("throws an error when validating invalid parent page object", async () => {
    await expect(async () => {
      await validateReportPayload(invalidParentPageReport);
    }).rejects.toThrow();
  });
  it("throws an error when validating invalid radio element checked children object", async () => {
    await expect(async () => {
      await validateReportPayload(invalidRadioCheckedChildrenReport);
    }).rejects.toThrow();
  });
  it("throws an error when validating invalid page element type", async () => {
    await expect(async () => {
...
describe("isReportOptions", () => {
  ...
  it("should accept an object with empty options", () => {
    const obj = buildValidReportOptions();
    obj.options = {};
    expect(isReportOptions(obj)).toBe(true);
  });
